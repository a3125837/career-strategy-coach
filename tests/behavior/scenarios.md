...
Supplemental comparison indicators (not baseline pass/fail):
- Explicitly states the age-reference stage separately from the actual task-completion stage.
- Labels the supporting evidence, unknowns, and confidence for the stage judgment.
...
Supplemental comparison indicators (not baseline pass/fail):
- Leads with the strongest counterargument.
- Gives a reversible validation or risk-reduction experiment with an explicit stop condition.
...
Supplemental comparison indicators (not baseline pass/fail):
- States the missing organization-specific evidence needed for the comparison.
- Suggests a low-cost management trial without starting a full intake.
